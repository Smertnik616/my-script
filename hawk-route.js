(function () {
    ...
    // ========== Конфіг Firebase ==========
    const DB_URL = 'https://script-poi-default-rtdb.europe-west1.firebasedatabase.app/rooms/hawk-route-default/points.json';
    const FLIGHTS_URL = 'https://script-poi-default-rtdb.europe-west1.firebasedatabase.app/rooms/hawk-route-default/flights.json';
    const FIREBASE_ENABLED = !DB_URL.includes('ВАШ_ПРОЄКТ');
    const STORAGE_KEY = 'cesium_hawk_route_points_v1';
    const SETTINGS_KEY = 'cesium_hawk_route_settings_v1';
    const CORRIDOR_KEY = 'cesium_hawk_route_corridor_v1';
    const CLIENT_KEY = 'hawk_route_client_id_v1';
    ...
    const FR_BUILD = 'hawk-copy-1';

    // Реєстр маркерів карти-хоста (треки/стрілки не з HawkRoute)
    ...
    function installHostTrackSpy() {
        ...
        console.log('[HAWKROUTE] host-track spy: markers will be tracked for attach');
        return true;
    }
    ...
    function boot(attempt) {
        ...
        console.warn('[HAWKROUTE] Автопошук не знайшов карту. Клацніть один раз по карті...');
        const tip = document.createElement('div');
        tip.id = 'hawk-route-tip';
        tip.textContent = '🦅 HAWKROUTE: клацніть один раз по карті для підключення';
    ...
    function initApp(engine) {
        ...
        console.log('[HAWKROUTE] init', FR_BUILD, mapType);
        ...
        const htmlLayout = `
            <style>
                #hawk-route-ui * { box-sizing: border-box; }
                #hawk-route-ui .fr-head {
                ...
                #hawk-route-ui .fr-head-title { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
                #hawk-route-ui .fr-head-name { font-weight: 750; font-size: 13px; letter-spacing: .02em; color: #e0f2fe; }
                #hawk-route-ui .fr-head-meta { font-size: 10px; color: #94a3b8; font-weight: 500; }
                #hawk-route-ui .fr-head-actions { display: flex; align-items: center; gap: 6px; }
                #hawk-route-ui .fr-icon-btn {
                ...
                #hawk-route-ui .fr-icon-btn:hover { background: #1e293b; color: #fff; border-color: #475569; }
                #hawk-route-ui .fr-body {
                ...
                #hawk-route-ui .fr-body.hidden { display: none; }
                #hawk-route-ui.fr-collapsed { height: auto !important; max-height: none; }
                #hawk-route-ui .fr-body > * { width: 100%; margin: 0; flex: none !important; flex-shrink: 0 !important; }
                #hawk-route-ui .fr-body > *:not(.fr-list) { max-height: none !important; }

                #hawk-route-ui input:not([type="checkbox"]):not([type="color"]),
                #hawk-route-ui select, #hawk-route-ui textarea, #hawk-route-ui button {
                ...
                #hawk-route-ui textarea:focus,
                #hawk-route-ui input:not([type="checkbox"]):not([type="color"]):focus,
                #hawk-route-ui select:focus {
                    outline: none; border-color: #38bdf8 !important; box-shadow: 0 0 0 2px rgba(56,189,248,.18);
                }
                #hawk-route-ui .fr-row { display: flex !important; justify-content: space-between; align-items: center; gap: 8px; min-height: 32px; }
                #hawk-route-ui .fr-row > label { flex: 0 0 auto; color: #94a3b8; font-size: 11px; }
                #hawk-route-ui input:not([type="checkbox"]):not([type="color"]) {
                ...
                #hawk-route-ui input[type="number"] { width: 84px; text-align: center; }
                #hawk-route-ui input[type="text"] { flex: 1 1 auto; min-width: 0; width: auto; }
                #hawk-route-ui input[type="color"] {
                ...
                #hawk-route-ui input[type="checkbox"] {
                ...
                #hawk-route-ui .fr-row select, #hawk-route-ui select { flex: 1 1 auto; min-width: 0; width: auto; }
                #hawk-route-ui .fr-grid { display: grid !important; grid-template-columns: 1fr 1fr; gap: 6px; }
                #hawk-route-ui .fr-grid-3 { display: grid !important; grid-template-columns: 1fr 1fr 1fr; gap: 6px; }

                #hawk-route-ui .fr-btn {
                ...
                #hawk-route-ui .fr-btn:hover { background: #273044; border-color: #3b82f6; color: #fff; }
                #hawk-route-ui .fr-btn-pick { background: #13233f; color: #7dd3fc; border: 1px solid #1d4ed8; width: 100%; }
                #hawk-route-ui .fr-btn-pick:hover { background: #1e3a5f; }
                #hawk-route-ui .fr-btn-pick.active { background: #b45309; color: #fff; border-color: #f59e0b; }
                #hawk-route-ui .fr-btn-danger { background: #2a1518; color: #fca5a5; border-color: #7f1d1d; }
                #hawk-route-ui .fr-btn-danger:hover { background: #dc2626; color: #fff; border-color: #ef4444; }
                #hawk-route-ui .fr-btn-wide { width: 100%; }
                #hawk-route-ui .fr-btn-ok { background: #10291c; color: #86efac; border-color: #166534; }
                #hawk-route-ui .fr-btn-ok:hover { background: #166534; color: #fff; }
                #hawk-route-ui .fr-btn-ok.active { background: #854d0e; color: #fef3c7; border-color: #eab308; }
                #hawk-route-ui .fr-btn-primary { background: #0ea5e9; color: #082f49; border-color: #38bdf8; font-weight: 750; }
                #hawk-route-ui .fr-btn-primary:hover { background: #38bdf8; color: #082f49; }

                #hawk-route-ui .fr-quick {
                    display: flex; flex-direction: column; gap: 6px;
                    padding: 8px; background: #171b26; border: 1px solid #2a3142; border-radius: 12px;
                }
                #hawk-route-ui .fr-qbar {
                    display: grid; grid-template-columns: repeat(5, 1fr); gap: 5px;
                }
                #hawk-route-ui .fr-qbtn {
                ...
                #hawk-route-ui .fr-qbtn:hover {
                    background: #1e293b; border-color: #38bdf8; color: #fff; transform: translateY(-1px);
                }
                #hawk-route-ui .fr-qbtn.active {
                    background: #b45309; border-color: #f59e0b; color: #fff;
                    box-shadow: 0 0 0 1px rgba(245,158,11,.35);
                }
                #hawk-route-ui .fr-qbtn.fr-q-ok.active {
                    background: #854d0e; border-color: #eab308; color: #fef3c7;
                }
                #hawk-route-ui .fr-qbtn.fr-q-go.active {
                    background: #166534; border-color: #4ade80; color: #fff;
                }
                #hawk-route-ui .fr-qbtn.fr-q-danger:hover {
                    background: #7f1d1d; border-color: #ef4444;
                }
                #hawk-route-ui .fr-qlabel {
                    font-size: 10px; color: #64748b; font-weight: 650; letter-spacing: .04em;
                    text-transform: uppercase; padding: 0 2px;
                }
                #hawk-route-ui .fr-quick-row {
                    display: grid; grid-template-columns: 1fr 1fr; gap: 6px;
                }
                #hawk-route-ui .fr-card {
                    background: #171b26; border: 1px solid #2a3142; border-radius: 12px;
                    padding: 0; overflow: hidden;
                }
                #hawk-route-ui .fr-card > .fr-card-body {
                    display: flex; flex-direction: column; gap: 8px; padding: 10px;
                }
                #hawk-route-ui details.fr-acc { background: #171b26; border: 1px solid #2a3142; border-radius: 12px; overflow: hidden; }
                #hawk-route-ui details.fr-acc > summary {
                    cursor: pointer; list-style: none; user-select: none;
                    display: flex; align-items: center; justify-content: space-between; gap: 8px;
                    padding: 10px 12px; background: #1a2030; color: #e2e8f0; font-weight: 700; font-size: 12px;
                }
                #hawk-route-ui details.fr-acc > summary::-webkit-details-marker { display: none; }
                #hawk-route-ui details.fr-acc > summary::after {
                    content: "▾"; color: #64748b; font-size: 11px; transition: transform .15s ease;
                }
                #hawk-route-ui details.fr-acc[open] > summary::after { transform: rotate(180deg); }
                #hawk-route-ui details.fr-acc > summary:hover { background: #20283a; }
                #hawk-route-ui details.fr-acc .fr-acc-body {
                    display: flex; flex-direction: column; gap: 8px; padding: 10px 12px 12px;
                    border-top: 1px solid #2a3142;
                }
                #hawk-route-ui .fr-acc-title { display: flex; align-items: center; gap: 8px; min-width: 0; }
                #hawk-route-ui .fr-acc-ico {
                ...
                #hawk-route-ui .fr-hint { color: #94a3b8; font-size: 10px; line-height: 1.4; }
                #hawk-route-ui .fr-status {
                ...
                #hawk-route-ui .fr-status.muted { color: #94a3b8; font-weight: 500; }
                #hawk-route-ui .fr-section { border-top: none; padding-top: 0; display: flex !important; flex-direction: column; gap: 8px; }
                #hawk-route-ui .fr-label { color: #94a3b8; font-size: 11px; }
                #hawk-route-ui .fr-count {
                    color: #93c5fd; font-size: 11px; font-weight: 700;
                    display: flex; justify-content: space-between; align-items: center;
                }
                #hawk-route-ui .fr-list {
                ...
                #hawk-route-ui .fr-item:last-child { border-bottom: none; }
                #hawk-route-ui .fr-item-main { flex: 1; min-width: 0; word-break: break-all; color: #cbd5e1; }
                #hawk-route-ui .fr-item-actions { display: flex; gap: 6px; flex-shrink: 0; }
                #hawk-route-ui .fr-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; margin-right: 4px; vertical-align: middle; }
                #hawk-route-ui .fr-sync {
                    font-size: 10px; color: #64748b; min-height: 14px; padding: 2px 2px 0;
                    display: flex; align-items: center; gap: 6px;
                }
                #hawk-route-ui .fr-sync::before {
                    content: ""; width: 7px; height: 7px; border-radius: 50%; background: #64748b; flex: 0 0 auto;
                }
                #hawk-route-ui .fr-sync.on { color: #4ade80; }
                #hawk-route-ui .fr-sync.on::before { background: #4ade80; box-shadow: 0 0 8px rgba(74,222,128,.55); }
                #hawk-route-ui .fr-sync.err { color: #f87171; }
                #hawk-route-ui .fr-sync.err::before { background: #f87171; }
                #hawk-route-ui .fr-legend { display: flex; flex-wrap: wrap; gap: 6px; }
                #hawk-route-ui .fr-legend span {
                    display: inline-flex; align-items: center; gap: 4px;
                    background: #0b0e14; border: 1px solid #2a3142; border-radius: 999px;
                    padding: 3px 8px; font-size: 10px; color: #cbd5e1;
                }
                #hawk-route-ui .fr-means-row { display: flex; gap: 4px; align-items: center; }
                #hawk-route-ui .fr-means-list { display: flex; flex-direction: column; gap: 4px; max-height: 110px; overflow-y: auto; }
                #hawk-route-ui details.fr-details > summary { cursor: pointer; color: #93c5fd; font-weight: bold; list-style: none; }
                #hawk-route-ui details.fr-details > summary::-webkit-details-marker { display: none; }
                #hawk-route-ui .fr-field-grid {
                    display: grid; grid-template-columns: 1fr 1fr; gap: 8px;
                }
                #hawk-route-ui .fr-field { display: flex; flex-direction: column; gap: 4px; }
                #hawk-route-ui .fr-field > label { color: #94a3b8; font-size: 10px; font-weight: 650; text-transform: uppercase; letter-spacing: .04em; }
                #hawk-route-ui .fr-field select,
                #hawk-route-ui .fr-field input:not([type="checkbox"]):not([type="color"]) { width: 100%; }
                #hawk-route-ui .fr-ruler-total { color: #7dd3fc; font-size: 11px; font-weight: bold; line-height: 1.35; }
                #hawk-route-ui .fr-footer {
            ...
            <div class="fr-head" id="fr-drag">
                <div class="fr-head-title">
                    <span class="fr-head-name">🦅 HawkRoute</span>
                    ...
                    <div class="fr-acc-body">
                        ...
                        <div class="fr-field-grid">
                            <div class="fr-field">
                                ...
                                <input type="text" id="fr-callsign" value="${settings.callsign || 'Hawk'}" maxlength="16" placeholder="Hawk">
                            </div>
                            <div class="fr-field">
        ...
        const ACC_KEY = 'hawk_route_ui_acc_v1';
        try {
        ...
        function saveSettings() {
            ...
            settings.callsign = (document.getElementById('fr-callsign')?.value || 'Hawk').trim().slice(0, 16) || 'Hawk';
        ...
        function parseImportedPayload(text, fileName) {
            ...
            if (json.type === 'HawkRoutePoints' && Array.isArray(json.points)) {
        ...
        async function pushToFirebase(points) {
            ...
            try {
                ...
                if (!res.ok || bodyJson?.error) {
                    ...
                    console.warn('[HAWKROUTE] Firebase PUT failed:', errMsg);
                    return;
                }
                ...
                syncEl.className = 'fr-sync err';
                console.warn('[HAWKROUTE] Firebase PUT failed:', err);
            }
        }
        ...
        async function copyMgrsAt(lat, lon, btn) {
            ...
            console.log('[HAWKROUTE] MGRS copied:', mgrs, '| DD:', dd);
            try {
                const tip = document.createElement('div');
        ...
        function pickHostTrackAt(lat, lon) {
            ...
            if (mapType === 'google') {
                ...
                console.warn('[HAWKROUTE] host tracks visible to spy:', n);
                return null;
            }
        ...
        async function pushMyFlight() {
            ...
            try {
                ...
                console.warn('[HAWKROUTE] flight PUT failed:', err);
            }
        }
        ...
        async function clearMyFlightRemote() {
            ...
            try {
                ...
                console.warn('[HAWKROUTE] flight DELETE failed:', err);
            }
        }
        ...
        function spawnOrMoveAircraft(lat, lon) {
            saveSettings();
            const callsign = (document.getElementById('fr-callsign').value || 'Hawk').trim().slice(0, 16) || 'Hawk';
            ...
            try {
                ...
                es.addEventListener('put', (e) => {
                    try {
                        ...
                        console.warn('[HAWKROUTE] flights sync parse error', err);
                    }
                });
                ...
                console.warn('[HAWKROUTE] flights EventSource failed', err);
            }
        }
        ...
        console.log(`🦅 HAWKROUTE v2 завантажено! (${mapType === 'google' ? 'Google Maps / R2D2' : 'Cesium'}) — лінійка/політ/синхрон`);
    }

    document.getElementById('hawk-route-ui')?.remove();
    document.getElementById('hawk-route-tip')?.remove();
    boot(0);
})();
